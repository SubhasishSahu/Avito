...
def _fetch_prices(yf_ticker: str, ticker: str) -> pd.Series | None:
    """Fetch price series with fallback chain."""
    import time
    for attempt, (t, period) in enumerate([
        ...
        try:
            df = yf.download(t, period=period, interval=PRICE_INTERVAL,
                             auto_adjust=True, progress=False, show_errors=False)
            if df is not None and not df.empty and len(df) > 10:
                close = df["Close"].squeeze()
                log.info(f"  {ticker}: {len(close)} rows (attempt {attempt+1})")
                return close
        except Exception:
            pass
        time.sleep(0.5)
    log.warning(f"  {ticker}: no price data after 3 attempts")
    return None
...
def harvest(tickers: list = None) -> dict:
    ...
    try:
        idx_df    = yf.download(NIFTY50_INDEX_YF, period=PRICE_PERIOD,
                                interval=PRICE_INTERVAL, auto_adjust=True,
