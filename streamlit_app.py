...
def tab_peers(enriched: pd.DataFrame, snapshot: dict):
    ...
    peers_df = pd.DataFrame(peers)[
        ["ticker", "name", "cagr_1y", "cagr_3y", "rsi", "beta_3y",
         "max_dd", "var_95", "alpha"]
    ].copy()
    peers_df.columns = ["Ticker", "Name", "CAGR 1Y%", "CAGR 3Y%",
                        "RSI", "Beta", "Max DD%", "VaR 95%", "Alpha%"]
...
def main():
    ...
    if uploaded is None:
        ...
        with st.expander("👀 Snapshot Preview (last 10 stocks)"):
            preview = pd.DataFrame(stocks)[
                ["ticker", "name", "sector", "price", "rsi", "macd",
